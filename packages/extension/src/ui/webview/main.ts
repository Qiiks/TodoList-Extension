...
(() => {
  ...
  const toElement = (value: EventTarget | null): HTMLElement | null =>
  ...
  const syncMarkdownViews = (): void => {
    const rendered = document.querySelectorAll<HTMLElement>('[data-md-rendered]');
    rendered.forEach((el) => {
  ...
  window.addEventListener('message', (event: MessageEvent) => {
    ...
    if (message.type === 'focus-quick-add') {
      const input = document.querySelector<HTMLElement>('.tt-quick-add-input');
      input?.focus();
      return;
    }
  ...
  document.addEventListener('submit', (event: Event) => {
    ...
    if (form.matches('[data-action="quick-add"]')) {
      ...
      const titleInput = form.querySelector<HTMLTextAreaElement>('[name="title"]');
  ...
  document.addEventListener('keydown', (event: KeyboardEvent) => {
    ...
    if (target?.matches('.tt-quick-add-input') && event.key === 'Enter' && !event.shiftKey) {
      event.preventDefault();
      const form = target.closest('form[data-action="quick-add"]') as HTMLFormElement | null;
      form?.requestSubmit();
    }

    if (event.key === 'Escape') {
      document.querySelectorAll<HTMLElement>('[data-todo-expanded]').forEach((el) => {
        el.hidden = true;
      });
    }
    ...
    const row = target?.closest('.tt-todo-row') as HTMLElement | null;
    if (row && (event.key === 'ArrowDown' || event.key === 'ArrowUp')) {
      event.preventDefault();
      const rows = Array.from(document.querySelectorAll<HTMLElement>('.tt-todo-row'));
  ...
  document.addEventListener('click', (event: MouseEvent) => {
    ...
    if (action === 'toggle-expand') {
      const expanded = document.querySelector<HTMLElement>(`[data-todo-expanded="${todoId}"]`);
      if (expanded) {
    ...
    if (action === 'add-checklist-item' && todoId) {
      const input = document.querySelector<HTMLInputElement>(`[data-checklist-input="${todoId}"]`);
      if (!input) {
        return;
      }
